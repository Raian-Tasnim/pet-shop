    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col lg:flex-row gap-10">
        ...
        <aside className="w-full lg:w-64 space-y-8 h-fit lg:sticky lg:top-24 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          <div>
            <h3 className="font-bold text-[#6F4E37] mb-4 text-lg flex items-center gap-2">
            ...
            <ul className="space-y-2 text-slate-600">
              {categories.map(item => (
                <li 
                  ...
                  className={`cursor-pointer transition px-3 py-2 rounded-lg 
                    ${selectedCategory === item 
                      ? 'bg-[#6F4E37] text-white font-bold' 
                      : 'hover:bg-[#F5F5DC] hover:text-[#6F4E37]'}`
                  }
                >
                  {item}
                </li>
              ...
              {filteredProducts.map((item) => (
                <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition duration-300 group relative">
                  ...
                  <div className="h-48 bg-[#F5F5DC] rounded-xl mb-4 flex items-center justify-center overflow-hidden relative">
                    ...
                    <img 
                      ...
                      className={`object-cover h-full w-full transition duration-500 
                        ${item.inStock ? 'opacity-90 group-hover:opacity-100 group-hover:scale-105' : 'grayscale opacity-50'}`
                      } 
                    />
                  ...
                  <div className="space-y-1">
                    ...
                    <h3 className={`font-bold text-lg leading-tight ${item.inStock ? 'text-slate-800' : 'text-slate-400'}`}>
                      {item.name}
                    </h3>
