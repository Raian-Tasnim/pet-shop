        ...
        {filteredPets.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {filteredPets.map((pet) => {
              ...
              // Check if this pet is in the favorites list passed from App.jsx
                ...
                <div key={pet.id} className="bg-white rounded-3xl p-4 shadow-sm hover:shadow-xl transition duration-300 group">
                  <div className="relative h-64 overflow-hidden rounded-2xl">
                    ...
                    <button 
                      ...
                      className="absolute top-4 right-4 bg-white/90 p-2 rounded-full hover:scale-110 transition duration-300 shadow-sm"
                    >
                      <Heart 
                        size={20} 
                        className={isFav ? "fill-red-500 text-red-500" : "text-[#bd4e0e]"} 
                      />
                    </button>
                  </div>
